...
@testset "Simple Scalar Function" begin
    ...
    a = @test_logs (:warn,"If you do not give outputs to the function then you can only give one vector of inputs (in a 2d array) to the fixed_pointFunction. So for a function that takes an N dimensional array you should input an Array{Float64}(N,1) array. As you have input an array of size Array{Float64}(N,k) with k > 1 we have discarded everything but the last column to turn it into an Array{Float64}(N,1) array.") fixed_point(func, fp_RRE.Inputs_; Algorithm = :RRE)
    ...
    b = @test_logs (:warn,"If you input a matrix of outputs as well as a matrix of inputs then inputs and outputs must be the same shape. As they differ in this case the last column of the inputs matrix has been taken as the starting point and everything else discarded for inputs of element type Float64.") fixed_point(func, fp_RRE.Inputs_; Outputs = fp_RRE.Outputs_[:,2:3], Algorithm = :RRE)
