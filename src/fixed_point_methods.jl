...
function fixed_point(
    func::Function,
    previous_FixedPointResults::FixedPointResults;
    Algorithm::Symbol=:Anderson,
    ConvergenceMetric::Function=supnorm(input, output) = maximum(abs.(output .- input)),
    ConvergenceMetricThreshold::Real=1e-10,
    MaxIter::Integer=Integer(1000),
    MaxM::Integer=Integer(10),
    ExtrapolationPeriod::Integer=Integer(7),
    Dampening::Number=1.0,
    Dampening_With_Input::Bool=false,
    PrintReports::Bool=false,
    ReportingSigFig::Integer=Integer(10),
    ReplaceInvalids::Symbol=:NoAction,
    ConditionNumberThreshold::Real=1e3,
...
function fixed_point(
    func::Function,
    Inputs::Array{T,2};
    Outputs::Array{<:Number,2}=Array{T,2}(undef, size(Inputs)[1], 0),
    Algorithm::Symbol=:Anderson,
    ConvergenceMetric::Function=supnorm(input, output) = maximum(abs.(output .- input)),
    ConvergenceMetricThreshold::Real=1e-10,
    MaxIter::Integer=Integer(1000),
    ...
    if (ConditionNumberThreshold < 1)
        error("ConditionNumberThreshold must be at least 1.")
    end
    ...
    while (Convergence > ConvergenceMetricThreshold) & (iter <= MaxIter)
        ...
        NewInputFunctionReturn = fixed_point_new_input(
            Inputs,
            Outputs,
            Algorithm;
            MaxM=MaxM,
            SimpleStartIndex=SimpleStartIndex,
            ExtrapolationPeriod=ExtrapolationPeriod,
            Dampening=Dampening,
            Dampening_With_Input=Dampening_With_Input,
            ConditionNumberThreshold=ConditionNumberThreshold,
