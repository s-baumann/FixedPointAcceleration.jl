...
function PolynomialExtrapolation(
    Iterates::AbstractArray{R,2}, Algorithm::Symbol
) where {R<:Number}
    if (Algorithm == :MPE)
        TotalColumnsOfIterates = size(Iterates)[2]
        OldDifferences =
            Iterates[:, 2:(TotalColumnsOfIterates - 1)] .-
            Iterates[:, 1:(TotalColumnsOfIterates - 2)]
        LastDifference =
            Iterates[:, TotalColumnsOfIterates] .- Iterates[:, (TotalColumnsOfIterates - 1)]
    ...
    elseif (Algorithm == :RRE)
        ...
        Differences =
            Iterates[:, 2:(TotalColumnsOfIterates)] -
            Iterates[:, 1:(TotalColumnsOfIterates - 1)]
        SecondDifferences =
            Differences[:, 2:(TotalColumnsOfIterates - 1)] -
            Differences[:, 1:(TotalColumnsOfIterates - 2)]
        ...
        error(
            "Invalid Algorithm input. PolynomialExtrapolation function can only take Algorithm as :MPE or :RRE.",
        )
    end
end
...
function EpsilonExtrapolation(
    ...
    if (size(Iterates)[2] % 2 == 0)
        Iterates = Iterates[:, 2:size(Iterates)[2]]
    end
    if (!(Algorithm in [:VEA, :SEA]))
        error(
            "Invalid Algorithm input. EpsilonExtrapolation function can only take Algorithm as VEA or SEA",
        )
    end
    Mat = Iterates
    ...
    for MatrixColumn in reverse(2:TotalColumnsOfMatrix)
        ...
        NewMatrix =
            PreviousMatrix + EpsilonExtrapolationVectorOfInverses(DiffMatrix, Algorithm)
...
"""
function EpsilonExtrapolationVectorOfInverses(
    DifferenceMatrix::AbstractArray{T,2}, Algorithm::Symbol
) where {T<:Number}
    if (size(DifferenceMatrix)[1] == 1) | (Algorithm == :SEA)
        return 1 ./ DifferenceMatrix
    else
        invs = transpose(pinv(DifferenceMatrix[:, 1]))
        if size(DifferenceMatrix)[2] < 2
            return invs
        end
