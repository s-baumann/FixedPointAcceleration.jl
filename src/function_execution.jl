...
"""
function execute_function_safely(
    Func::Function, x::Array{T,1}; type_check::Bool=false, quiet_errors::Bool=true
) where {T<:Number}
    ...
    elseif isa(tf_full_result, Tuple) &&
        (length(tf_full_result) == 2) &&
        ((isa(tf_full_result[1], Vector) & isa(tf_full_result[2], NamedTuple)))
        tf_result = tf_full_result[1]
        side_effect_to_report = tf_full_result[2]
    else
        error(
            "This function returned a $(typeof(tf_full_result)). The Fixedpoint function can only return a vector or a tuple of which the first entry is the vector for which a fixedpoint is sought and the second is a namedtuple (the contents of which are output for the user but are not used in fixed point acceleration).",
        )
    end
    # Check Output and return - handle complex numbers properly
    if sum(ismissing.(tf_result)) > 0
        return FunctionEvaluationResult(
            x, tf_result, :OutputMissingsDetected, side_effect_to_report
        )
    elseif eltype(tf_result) <: Complex
        ...
        if any(isnan.(real.(tf_result))) || any(isnan.(imag.(tf_result)))
            return FunctionEvaluationResult(
                x, tf_result, :OutputNAsDetected, side_effect_to_report
            )
        elseif any(isinf.(real.(tf_result))) || any(isinf.(imag.(tf_result)))
            return FunctionEvaluationResult(
                x, tf_result, :OutputInfsDetected, side_effect_to_report
            )
        end
    else
    ...
    if (length(tf_result) != length(x))
        return FunctionEvaluationResult(
            x, tf_result, :LengthOfOutputNotSameAsInput, side_effect_to_report
        )
    elseif type_check & (!isa(tf_result[1], T))
        return FunctionEvaluationResult(
            x, tf_result, :FunctionIsNotTypeStable, side_effect_to_report
        )
    else
