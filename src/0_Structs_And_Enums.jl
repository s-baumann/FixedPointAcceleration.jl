...
struct FunctionEvaluationResult{RR<:Real,TT}
    ...
    function FunctionEvaluationResult(Input::Array{T,1}, Output_::Missing, Error_::FP_FunctionEvaluationError) where T<:Real
        return new{T,T}(Input, Output_, Error_)
    end
    function FunctionEvaluationResult(Input::Array{T,1}, Output::Array{R,1}, Error_::FP_FunctionEvaluationError) where T<:Real where R<:Real
        return new{T,R}(Input, convert(Array{Union{Missing,R},1}, Output), Error_)
    end
    function FunctionEvaluationResult(Input::Array{T,1}, Output::Array{Union{Missing,R},1}, Error_::FP_FunctionEvaluationError) where T<:Real where R<:Real
        return new{T,R}(Input, Output, Error_)
    end
end
...
struct FixedPointResults{R<:Real}
    ...
    function FixedPointResults(Inputs_::Array{R,2}, Outputs_::Array{R,2}, TerminationCondition_::TerminationCondition;
                               ...
                               FailedEvaluation_::Union{Missing,FunctionEvaluationResult} = missing) where R<:Real
        ...
        return new{R}(FixedPoint_, Convergence_, TerminationCondition_, Iterations_, ConvergenceVector_, FailedEvaluation_, Inputs_, Outputs_)
    end
end
